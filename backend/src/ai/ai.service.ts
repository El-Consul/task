  ...
  async getChatResponse(prompt: string, _userId: string) {
    try {
      const response = await this.openai.chat.completions.create({
        model:
          this.configService.get<string>('AI_MODEL') || 'qwen2.5-coder:1.5b',
        messages: [
          {
            role: 'system',
            content:
              'You are a helpful assistant for a Real Estate Management System. Help the user with client data, payments, and system questions.',
          },
