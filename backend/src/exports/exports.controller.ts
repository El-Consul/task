...
export class ExportsController {
  ...
  async export(@Req() req: any, @Res() res: Response, @Query() filters: any) {
    ...
    res.setHeader(
      'Content-Disposition',
      `attachment; filename=accounting_${Date.now()}.xlsx`,
    );
  ...
  async exportYearly(
    ...
    res.setHeader(
      'Content-Disposition',
      `attachment; filename=yearly_${y}.xlsx`,
    );
  ...
  async exportClient(
    ...
    res.setHeader(
      'Content-Disposition',
      `attachment; filename=client_${id}.xlsx`,
    );
  ...
  async exportUnits(@Req() req: any, @Res() res: Response) {
    ...
    res.setHeader(
      'Content-Disposition',
      `attachment; filename=units_status.xlsx`,
    );
